...
class Prompt_block(nn.Module, ):
    def __init__(self, inplanes=None, hide_channel=None, smooth=False):
        ...
        self.conv0_2 = nn.Conv2d(in_channels=inplanes, out_channels=4, kernel_size=1, stride=1, padding=0)
        self.conv0_3 = nn.Conv2d(in_channels=inplanes, out_channels=4, kernel_size=1, stride=1, padding=0)
    ...
    def forward(self, x):
        ...
        B, C, W, H = x.shape
        if C == 2304:
           x_rgb = x[:, 0:int(C / 3), :, :].contiguous()
           x_rgb = self.conv0_0(x_rgb)
           x_depth = x[:, int(C / 3):1536, :, :].contiguous()
           x_depth = self.conv0_2(x_depth)
           x_infrared = x[:, 1536:, :, :].contiguous()
           x_infrared = self.conv0_3(x_infrared)
           x_dt = torch.cat((x_depth, x_infrared),dim=1)
           x0 = self.fovea(x_rgb) + x_dt
        else:
...
class VisionTransformerCE(VisionTransformer):
    ...
    def forward_features(self, z, x, mask_z=None, mask_x=None,
        ...
        if self.prompt_type in ['rdtt_shaw', 'rdtt_deep']:
            ...
            z_feat = torch.cat([z_feat, z_depth_feat, z_thermal_feat], dim=1)
            x_feat = torch.cat([x_feat, x_depth_feat, x_thermal_feat], dim=1)
